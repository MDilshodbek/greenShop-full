...
import { useViewOrderRequest } from "./features";
import TrackingCard from "./customs/card";

const ViewOrder = () => {
  const {
    trackModal: { show, data },
  } = useSelector(({ modal }) => modal);
  const { onCancelRequest, onDelete } = useViewOrderRequest();

  const total = data?.extra_shop_info.total_price ?? 0;
  const hasCoupon = data?.extra_shop_info.coupon.has_coupon ?? false;
  const discount_for = data?.extra_shop_info.coupon.discount_for ?? false;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-3">
        ...
        {/* <TrackingCard key={value?._id} {...value} /> */}
      </div>
